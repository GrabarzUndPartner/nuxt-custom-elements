...
function getWebpackConfig (entryName, nuxt, config, options) {
  ...
  fixCSSLoaderForExtractCSS(nuxt, config);
 ...
 */
function fixCSSLoaderForExtractCSS (nuxt, config) {
  if (nuxt.options.build.extractCSS) {
    config.module.rules.forEach(rule => rule.oneOf && rule.oneOf.forEach(oneOf => oneOf.use && oneOf.use.forEach((use) => {
