...
import { getBuildDir, MODULE_NAME } from '../utils'
import { getTagHTMLFromEntry } from './tags'

const AVAILABLE_CONFIG_NAMES = ['client', 'modern']
const TEMPLATE = path.join(__dirname, '..', 'tmpl', 'index.html.tmpl')
...
function createHtmlWebpackPlugin (chunks, tags) {
  let filename = 'index.html'
  if (chunks.length === 1) {
    // filename = path.join(chunks[0], filename)
...
function createHtmlWebpackPlugins (entries) {
  const tags = entries.map(entry => createHtmlWebpackPlugin([entry.name], getTagHTMLFromEntry(entry)))
  tags.unshift(createHtmlWebpackPlugin(entries.map(entry => entry.name), entries.reduce((result, entry) => {
