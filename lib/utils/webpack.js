...
export function getConfigs (builder, options) {
  ...
  return Promise.all(Object.keys(options.entry).map((entryName, i) => {
    return getNuxtWebpackConfigs(builder).then((configs) => {
      return configs
        .filter(config => configNames.includes(config.name))
        .map((config) => {
          return getWebpackConfig(entryName, builder.nuxt, config, Object.assign({}, options, { entry: { [entryName]: options.entry[entryName] } }))
        })
    })
  }))
}
...
export function getWebpackConfig (entryName, nuxt, config, options) {
  ...
  const filename = options.webpackOutput.filename

  let outputFunc
  if (typeof filename === 'function') {
    outputFunc = function (chunk) { return filename(chunk, config, options) }
  }

  return Object.assign({}, config, {
    target: 'web',
    entry: prepareEntries(options, isModernBuild),
    output: Object.assign({
      publicPath: './'
    }, options.webpackOutput, {
      filename: outputFunc || filename,
...
function prepareEntries (options, isModernBuild) {
  return Object.fromEntries(Object.keys(options.entry).map(key => [key, options.entry[String(key)][isModernBuild ? 'modern' : 'client']]))
}
