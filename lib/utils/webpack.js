...
function setLoaderRulesForShadowMode (rules, active = true) {
  const vueLoaderRule = rules.find(({ test }) => test.test('.vue'));
  if (vueLoaderRule) {
    if (active) {
      vueLoaderRule.options.shadowMode = active;
    } else {
      delete vueLoaderRule.options.shadowMode;
    }
  } else {
  ...
  const vueStyleLoaderDefinitions = ruleSets.reduce((result, name) => [
    ...
    .filter(Boolean)
    .map(v => v?.oneOf.find(v => Object.keys(v).length === 1)?.use[0]);
  if (vueStyleLoaderDefinitions.length) {
    vueStyleLoaderDefinitions.forEach(({ options }) => (options.shadowMode = active));
  } else {
...
function getWebpackConfig (entryName, nuxt, config, options) {
  ...
  rules = setLoaderRulesForShadowMode(rules, options.entries.find(({ name }) => name === entryName).shadow || false);
