...
> ⚠️&nbsp;&nbsp;Polyfill not included in the `modern` build, optional include by option `modernPolyfill`.
...
| `modernPolyfill`          | `Boolean`         | Set `true` for all browsers that require a polyfill for [custom elements](https://github.com/ungap/custom-elements). Example: In `IE Edge 4x` will load the `modern` scripts.                                                                       | `false`                                                      | `false`  |
...
| `webpackPublicPathInject` | `Function`        | Inject webpack public path over entry file. `Function` call client side. e.g. `() => global.customPublicPath` or `function () { return global.customPublicPath; }`                                                                                  | `undefined`                                                  | `false`  |
